...
select  count(distinct(city)) number_of_cities from Home_Loan_Data
where joint_loan = 1
...
select property_type,max(property_value) max_property_value from Home_Loan_Data
...
select bd.banker_id,first_name,last_name,count(distinct(loan_id)) transaction_count from Banker_Data bd join Loan_Records_Data ld
...
select avg(datediff(YEAR,dob,transaction_date)) age from Customer_Data cd join Loan_Records_Data ld
on ld.customer_id = cd.customer_id join Home_Loan_Data hd
...
select count(city) total_city from Home_Loan_Data
...
select  top 3 city,avg(loan_percent) avg_loan_percent from Home_Loan_Data
group by city
...
select avg(datediff(YEAR,dob,date_joined)) avg_age from Banker_Data
where gender = 'Male'
...
select (city),AVG(property_value) avg_property_value from Home_Loan_Data
...
select count (nationality ) total_customers from Home_Loan_Data hd join Loan_Records_Data ld
on ld.loan_id = hd.loan_id join Customer_Data cd
on cd.customer_id = ld.customer_id join Banker_Data bd
on bd.banker_id = ld.banker_id
...
select city,property_type,(property_value * loan_percent / 100)  loan_amt from Home_Loan_Data
...
select top 3 transaction_date,sum(property_value * loan_percent / 100) sum_loan_amt from Home_Loan_Data hd join Loan_Records_Data ld
on hd.loan_id = ld.loan_id  
...
select bd.banker_id ,sum(property_value * loan_percent / 100) laon_amt from Banker_Data bd join Loan_Records_Data ld
on bd.banker_id = ld.banker_id join Home_Loan_Data hd
on hd.loan_id = ld.loan_id
...
select bd.banker_id,(property_value * loan_percent / 100) loan_amt from Banker_Data bd join Loan_Records_Data ld
on ld.banker_id = bd.banker_id join Home_Loan_Data hd
on hd.loan_id = ld.loan_id
...
select banker_id,CONCAT(first_name,' ',last_name) fullname,dob,date_joined from Banker_Data
...
select cd.customer_id,CONCAT(cd.first_name,' ',cd.last_name) fullname  from Customer_Data cd join Loan_Records_Data ld
on ld.customer_id = cd.customer_id join Banker_Data bd
on bd.banker_id = ld.banker_id
